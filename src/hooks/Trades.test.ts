...
describe('Trade', () => {
  describe('#useAllCommonPairs', () => {
    ...
    it('should filter only exist Pair', () => {
      mockUsePairs.mockReturnValue([
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.wbnb, '1'), new TokenAmount(mainnetTokens.megg, '1')),
        ],
        [
          PairState.INVALID,
          new Pair(new TokenAmount(mainnetTokens.busd, '1'), new TokenAmount(mainnetTokens.megg, '1')),
        ],
        [
          PairState.LOADING,
      ...
      const { result } = renderHook(() => {
        const pairs = Trades.useAllCommonPairs(mainnetTokens.wbnb, mainnetTokens.megg)
        return {
          pairs,
        }
      })
    ...
    it('should filter out duplicated Pair', () => {
      mockUsePairs.mockReturnValue([
        ...
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.megg, '1'), new TokenAmount(mainnetTokens.wbnb, '1')),
        ],
        [PairState.EXISTS, null],
      ])
    ...
    it('should get all pair combinations wbnb, megg', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(mainnetTokens.wbnb, mainnetTokens.megg)
      })
  ...
  describe('#useTradeExactIn/Out', () => {
    ...
    it('should call with maxHops 1 with singleHopOnly', () => {
      const allowPairs = [new Pair(new TokenAmount(mainnetTokens.wbnb, '1'), new TokenAmount(mainnetTokens.megg, '1'))]
      const argA = CurrencyAmount.ether('1000000')
      const argB = mainnetTokens.megg
      renderHook(
        () => {
